...
node_generate_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            """
            아래 규칙을 반드시 지켜라.
            - 너는 KB손해보험 자동차보험 약관만을 근거로 답하는 친절한 보험 전문가다.
            - CONTEXT 밖 정보를 추측하거나 새 보험 상품을 만들어내지 마라.
            - CONTEXT에 질문과 직접 관련된 조항이 있을 때만 그 상품과 조항을 설명하라.
            - CONTEXT에 해당 정보가 없거나 질문이 무관하면, 아무 설명 없이 '잘 모르겠습니다.'라고만 답한다.
            - 답변은 한국어로 하며, 필요한 경우 상품명·조항번호를 괄호로 명시하라.
            -------
            CONTEXT:
            {context}
            """,
...
enrich_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            """
            아래 규칙을 반드시 지켜라.
            - 기존 답변의 핵심을 유지하며, 전문 용어나 조항을 한국어로 쉽게 풀어서 제공하라.
            - CONTEXT 밖 정보를 추가하지 말아라.
            - CONTEXT가 비었거나 기존 답변이 '잘 모르겠습니다.'면 동일 문장을 그대로 반환하라.
            -------
            CONTEXT:
            {context}
            """,
...
def run_rag(question: str) -> str:
    """질문을 받아 LangGraph 워크플로를 실행하고 답변 반환."""
